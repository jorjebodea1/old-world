...
namespace gps {
	...
	void Model3DOBJ::LoadModel(std::filesystem::path path) {
		path=std::filesystem::absolute(path);
		modelFile = path.filename().string();
		ReadOBJ(path.string(), path.parent_path().string() + "\\");
	}

	void Model3DOBJ::LoadModel(std::string fileName) {
		modelFile = fileName;
        std::string basePath = fileName.substr(0, fileName.find_last_of('/')) + "/";
		ReadOBJ(fileName, basePath);
	}

    void Model3DOBJ::LoadModel(std::string fileName, std::string basePath)	{
		modelFile = fileName;
		ReadOBJ(fileName, basePath);
	...
	void Model3DOBJ::Draw(gps::Shader shaderProgram) {
	...
	void Model3DOBJ::ReadOBJ(std::string fileName, std::string basePath) {
	...
	gps::Texture Model3DOBJ::LoadTexture(std::string path, std::string type) {
	...
	GLuint Model3DOBJ::ReadTextureFromFile(const char* file_name) {
		int x, y, n;
	...
	Model3DOBJ::~Model3DOBJ() {
        ...
        for (size_t i = 0; i < meshes.size(); i++) {
            ...
            GLuint VBO = meshes.at(i).getBuffers().vertexVBO;
	...
	std::string Model3DOBJ::getModelFile() {
		return modelFile;
	}
}
